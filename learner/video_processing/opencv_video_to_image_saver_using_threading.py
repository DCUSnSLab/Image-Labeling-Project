...
키보드 단축키:
    * 버튼 누르는 방법에 순서가 없슴닷! 조작 상태를 볼려면 디버깅 창(tab)을 활성화 해주세욯
...
IMAGE_SAVE_PATH = u'C:/Users/yooer/Downloads/test/'
# VIDEO_FILE_PATH = u'VIDEO_PATH' # 실행시킬 비디오파이이일!
#IMAGE_SAVE_PATH = u'SAVE_PATH' # 저장될 파일 경로~! 원드라이브 경로가 안먹히는데... 이유를 모르겠다 ;ㅂ;...
...
IS_PLAY_NOT_PAUSED = False # 일시정지/재생 플레그
...
class TextOnScreen(object):
    ...
    아 그리고 지금은 한글 지원 안되요....;ㅅ; cv.putText 함수가 ASCII만 지원해요..
    """
    ...
    def write_on_frame(self, frame):
        ...
        for itr_container in range(0, len(self._container)): # 출력할 디버깅 메세지들을 하나씩 꺼내서 프레임에 씁니다
            ...
            # 디버깅 메세지 쓰기~!
            cv.putText(_frame, _tmp_str.decode('utf-8'), (self.display_x + 1, self.display_y + 1 + _interval), cv.FONT_HERSHEY_PLAIN, self.display_text_size, (0, 0, 0), thickness = self.display_text_thickness, lineType=cv.LINE_AA)
...
def process_frame(frame, frame_counter):
    ...
    영상을 저장하는 기능 추가!~! 단, 폴더 경로가 Microsoft Onedrive경로를 포함하는 경우나 기타 경우에 의해서 안 될 수 있으니
    실제로 저장되는지 한번 확인해볼 필요가 있음이!
    ...
    # ret, frame = cv.threshold(gray,0,255,cv.THRESH_BINARY_INV+cv.THRESH_OTSU)
...
if __name__ == '__main__':
    _PAUSED_FRAME = None # 일시정지된 프레임을 담는 공간~
    ...
    비디오 재생 시작
    """
    while VIDEO.isOpened() is True:
        """
        사용자 키 입력받기
        """
        USER_INPUT = cv.waitKey(1) # 키 입력 대기 (밀리초 기준)
        ...
        if USER_INPUT is ord('s'): # 이미지 저장기능 활성화
            IS_SAVE_IMAGE_ENABLE = not IS_SAVE_IMAGE_ENABLE
        if USER_INPUT is 27 or USER_INPUT is ord('q'): # 프로그램 끄기 (esc키, q키 )
            break
        if USER_INPUT is ord(' '): # 재생 일시 정지
            IS_PLAY_NOT_PAUSED = not IS_PLAY_NOT_PAUSED
        
        """
        비디오 프래임 재생 구간
        """
        if IS_PLAY_NOT_PAUSED is True: # 화면 재생하기~!
            if len(THREAD_CONTAINER) < THREAD_COUNTS: # 스레드에 공간이 있다면 작업 넣기~
             #일시정지 안 했을때만 작업하기~
                ...
                if IS_THREAD_ENABLE is True: # 스레드 사용하면 프로세스 여러개 사용하기
                    THREAD_TASKS = THREAD_POOL.apply_async(process_frame, (frame.copy(), FRAME_COUNTER)) # process_frame 함수를 거처서 결과를 가져오기~
                else: # 스레드가 꺼져있으면 그냥 프로세스 하나로 작업하기
                    THREAD_TASKS = DummyThreadTask(process_frame(frame, FRAME_COUNTER)) # process_frame 함수를 거처서 결과를 가져오기~
                FRAME_COUNTER += 1 # 프레임 번호 증가
            ...
            while len(THREAD_CONTAINER) > 0 and THREAD_CONTAINER[0].ready(): # 작업해야될 스레드 개수가 다 찻다면 화면 잠깐 멈추고 출력하기
                ...
                cv.imshow(WINDOW_TITLE, DEBUGGER.write_on_frame(frame)) # 화면 보여주기!
